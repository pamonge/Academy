...
class RegisterForm (UserCreationForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email_field).exists():
            raise forms.ValidationError('La dirección de correo ya se encuentra registrada')
        return email_field
...
class CourseForm (forms.ModelForm):
    ...
    description = forms.CharField(widget=forms.Textarea(attrs={'rows':5}), label="Descripción")
